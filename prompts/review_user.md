...
3. **Unverified assumptions**: Include only assumptions that pose significant uncertainty or risk,
   ...
   - Examples worth listing: breaking changes to external APIs, incompatible schema changes, missing
     required configuration that would cause runtime failures.
   - Do not list: routine function calls, standard library usage, or typical dependencies expected
     to work.
